...
void Body::damageBody(short damage)
{
	...
	m_p_lastDamageTaken_->setStartPoint(SDL_GetTicks());
	m_p_lastFrame_->setStartPoint(0); //instantly print a new frame
	m_currentSprite_ = 0;
}
...
bool Body::isInvincible()
{
	...
	if (!m_p_lastDamageTaken_->checkClockState()) {
		return true;
	}
	else
		return false;
...
Body::Body(SDL_FRect m_bounds_, SDL_FRect m_spriteBounds_, short m_maxLives_) : Entity(m_spriteBounds_)
{
	...
	m_p_lastFrame_->setStartPoint(0); // trigger new frame
	m_p_lastDamageTaken_ = new Clock(1000, false); //Every spawned Body can take damage
	m_p_lastDamageTaken_->setStartPoint(0);
}
